...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-50 border-b border-gray-100 bg-white/90 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 flex items-center justify-between h-16">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-emerald-600 rounded-xl flex items-center justify-center shadow shadow-emerald-200">
      ...
      <section className="py-24 px-5 sm:px-8">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <h2 className="text-4xl font-black text-gray-900">14 features, one dashboard</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {features.map(({ icon: Icon, title, desc, bg, iconColor }) => (
              ...
              >
                <div className={`w-9 h-9 ${bg} rounded-xl flex items-center justify-center mb-3.5`}>
      ...
      {/* ── Why self-host strip ── */}
      <section className="border-y border-gray-100 bg-gray-50 py-12 px-5 sm:px-8">
                ...
                <ul className="mt-7 space-y-3">
                  {plan.features.map((f) => (
                    <li key={f} className={`flex items-start gap-2.5 text-sm font-medium ${plan.primary ? "text-gray-300" : "text-gray-700"}`}>
                      <CheckCircle2 className={`w-4 h-4 shrink-0 mt-0.5 ${plan.primary ? "text-emerald-400" : "text-emerald-600"}`} />
                      {f}
                    </li>
                  ))}
                </ul>
